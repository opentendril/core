...
| `AuthMiddleware` | HTTP middleware authenticating the Botanist key (`BOTANIST_KEY`) and rejecting un-gated delegated requests. |
...
- **`AuthMiddleware` fails open when `BOTANIST_KEY` is unset (`cmd/stem/internal/receptors/config.go`):** When the `BOTANIST_KEY` environment variable is empty or unset, `AuthMiddleware` skips bearer token validation for non-delegated HTTP requests on `ConfigHandler` endpoints (`/v1/config/triggers`, `/v1/config/genotypes`). Unauthenticated callers can list and upload triggers or genotypes when `BOTANIST_KEY` is omitted.
