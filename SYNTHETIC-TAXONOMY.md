...
*   **Sprout**: The ephemeral Docker terrarium or worker environment. A sprout is a brand new, isolated "leaf" or "bud" of growth. In OpenTendril, every time a Transcript executes, a fresh, disposable container (the Sprout) is created, providing a clean, isolated environment that withers away upon completion.
*   **Tendril (The Modular Connector)**: In botany, a tendril is a specialized appendage used by climbing plants to *reach out and attach* to external structures for support. In OpenTendril, a Tendril is a modular enterprise plugin or integration (e.g., an MCP sub-server or Go interface). Rather than bloating the Stem with external SDKs (like GCP or AWS), the Tendril reaches out and grips external systems.
*   **Fruit (The Output Deliverables)**: The final product of the Sprout's lifecycle. Once a Sprout reaches reproductive maturity and its task is validated, it produces Fruit in the form of deliverables (e.g., Pull Requests, compiled artifacts, or system changes).
