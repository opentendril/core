...
| **Fruit** | Final Deliverables / Output | The final produced Artifacts, Pull Requests, or compiled deliverables delivered by the Sprout at maturity. |
...
| **Sprout** | Ephemeral Worker / Container | A stateless, disposable container (the "leaf" or "bud") that spins up for a single task and withers away upon completion. |
| **Stem** | Core Orchestrator | The lean, deterministic Go daemon. It enforces sequence rules and isolation boundaries but has no business logic or LLM "smarts". |
...
| **Tendril** | Modular Connector / Enterprise Plugin | A specialized integration module that reaches out and attaches to external systems (e.g., AWS, GCP) without bloating the Stem. |
...
| **Backend Server / Orchestrator** | **Stem** | The lean, central Go process managing orchestration rules and boundaries. |
| **Docker Container** | **Sprout** (or **Terrarium**) | The ephemeral, stateless execution sandbox (the "leaf"). |
| **Final Deliverables / Output** | **Fruit** | The final Artifacts or Pull Requests produced by the Sprout. |
...
| **Worker Script / Agent** | **Tendril** | A modular connector or enterprise integration plugin reaching external systems. |
