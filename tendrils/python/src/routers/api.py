...
from ..dependencies import llm_router, memory, tendrilloop, approval
...
Respond with ONLY the complete new file content. No explanations, no markdown fences."""
        ...
        if SDLC_MODE == "strict":
            ...
            if req.file.endswith(".py"):
                ...
                lint_output = await tendrilloop.tester.run_command(lint_cmd, safe=True)
                if "❌" in lint_output and "Command failed" in lint_output:
                    ...
                    test_output = await tendrilloop.tester.run_command(test_cmd, safe=True)
                    if "❌" in test_output:
                        test_passed = False
        ...
        if approval_status in ("approved", "auto_approved"):
            ...
            if SDLC_STRATEGY == "pr":
                current_branch = tendrilloop.git.current_branch()
                if current_branch == "main":
                    branch_name = f"tendril-patch-{uuid.uuid4().hex[:8]}"
                    tendrilloop.git.create_branch(branch_name)
                    logger.info(f"Created new branch for PR: {branch_name}")
                
                git_result = tendrilloop.git.commit_changes(commit_msg)
                push_result = tendrilloop.git.push_branch()
                ...
                try:
                    ...
                    # We can use tendrilloop.git.create_pull_request directly if we know the repo.
                    ...
                    remote_url = tendrilloop.git._run_git("config", "--get", "remote.origin.url")
                    repo_name = ""
                    ...
                    if repo_name:
                        pr_result = tendrilloop.git.create_pull_request(repo_name, pr_title, pr_body, tendrilloop.git.current_branch())
                    else:
    ...
    try:
        ...
        if intercepted:
            ...
            response = await asyncio.to_thread(
                tendrilloop.process, req.session_id, req.message, provider=prov
            )
    ...
    try:
        ...
        if req.stream:
            async def event_generator():
                ...
                try:
                    task = asyncio.create_task(
                        asyncio.to_thread(tendrilloop.process, session_id, user_msg, provider=provider)
                    )
                    while True:
            ...
            response_text = await asyncio.to_thread(
                tendrilloop.process, session_id, user_msg, provider=provider
            )
            memory.store_convo(session_id, "user", user_msg)
